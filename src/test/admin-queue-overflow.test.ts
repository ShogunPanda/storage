...
import { PgExecutor, PgPoolExecutor } from '../internal/database/pg-connection'
...
async function findJob(
  ...
  name: string,
  id: string,
  db: PgExecutor | undefined = queryDb
...
async function cleanupTestJobs(db: PgExecutor | undefined = queryDb) {
  if (!db) {
    return
  }
...
describe('Admin queue overflow routes', () => {
  ...
  it('rolls back the backup delete when restoring into the live table fails', async () => {
  ...
  it('restores backed up jobs in deterministic batches', async () => {
  ...
  it('drops archived conflicts while preserving the live job', async () => {
  ...
  it('moves only the matching tuple through a partitioned pg-boss table', async () => {
    ...
    try {
      ...
      await expect(store.backup({ name: 'exactly-once' })).resolves.toMatchObject({ movedCount: 1 })
      await boss.send('exactly-once', {}, { id: replacementId, singletonKey: 'tenant-a' })

      await expect(store.restore({ name: 'exactly-once', limit: 1 })).resolves.toMatchObject({
        conflictCount: 1,
        hasMore: true,
        movedCount: 0,
      })
      ...
      await expect(findJob(jobTable, 'exactly-once', replacementId, queryDb)).resolves.toBeDefined()
    } finally {
